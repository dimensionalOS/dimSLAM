...
pub fn reproject_depth(
    ...
    let (fx_d, fy_d) = (depth_info.intrinsics[0], depth_info.intrinsics[4]);
    let (cx_d, cy_d) = (depth_info.intrinsics[2], depth_info.intrinsics[5]);
    let (fx_c, fy_c) = (camera_info.intrinsics[0], camera_info.intrinsics[4]);
    let (cx_c, cy_c) = (camera_info.intrinsics[2], camera_info.intrinsics[5]);
...
mod tests {
    ...
    fn make_info(width: i32, height: i32, focal: f64) -> CameraModel {
        let mut info = CameraModel {
            width,
            height,
            ..Default::default()
        };
        info.intrinsics[0] = focal;
        info.intrinsics[4] = focal;
        info.intrinsics[2] = width as f64 / 2.0;
        info.intrinsics[5] = height as f64 / 2.0;
        info
    }

    fn make_depth(width: i32, height: i32) -> ImageFrame {
        ImageFrame {
            width,
            height,
            step: width * 2,
    ...
    fn set_pixel(image: &mut ImageFrame, u: i32, v: i32, value: u16) {
        let start = (v * image.step + u * 2) as usize;
        image.data[start..start + 2].copy_from_slice(&value.to_ne_bytes());
    }

    fn get_pixel(image: &ImageFrame, u: i32, v: i32) -> u16 {
        let start = (v * image.step + u * 2) as usize;
    ...
    fn nearest_surface_wins_on_collision() {
        ...
        target_info.intrinsics[2] = 0.0;
        target_info.intrinsics[5] = 0.0;
